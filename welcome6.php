...
<form method="post">
	<table class="table">
		<Tr>
			...
			<td>
				<select name="txannual" class="form-control">
					<option>select</option>
					<?php
						$q=mysqli_query($con,"select distinct annual_income from regtable");
						while ($r=mysqli_fetch_array($q)) {
							?>
							<option value="<?php echo $r['annual_income'];?>"><?php echo $r["annual_income"];?></option>
							<?php
						}
					?>
				</select>
...
if(isset($_POST['btnsearch'])){
	extract($_POST);
$q=mysqli_query($con,"select * from regtable where annual_income= '".$txannual."' and gender !='".$_SESSION['gender']."'");
}
else{
