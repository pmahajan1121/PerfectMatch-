...
<form method="post">
	<table class="table">
		<Tr>
			...
			<td>
				<select name="rbdegree" class="form-control">
					<option>select</option>
					<?php
						$q=mysqli_query($con,"select distinct degree from regtable");
						while ($r=mysqli_fetch_array($q)) {
							?>
							<option value="<?php echo $r['degree'];?>"><?php echo $r["degree"];?></option>
							<?php
						}
					?>
				</select>
...
if(isset($_POST['btnsearch'])){
	extract($_POST);
$q=mysqli_query($con,"select * from regtable where degree = '".$rbdegree."' and gender !='".$_SESSION['gender']."'");
}
else{
