...
<form method="post">
	<table class="table">
		<Tr>
			...
			<td>
				<select name="rbstatus" class="form-control">
					<option>select</option>
					<?php
						$q=mysqli_query($con,"select distinct marital_status from regtable");
						while ($r=mysqli_fetch_array($q)) {
							?>
							<option value="<?php echo $r['marital_status'];?>"><?php echo $r["marital_status"];?></option>
							<?php
						}
					?>
				</select>
...
if(isset($_POST['btnsearch'])){
	extract($_POST);
$q=mysqli_query($con,"select * from regtable where marital_status = '".$rbstatus."' and gender !='".$_SESSION['gender']."'");
}
else{
